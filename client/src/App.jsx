...
<div className="cd"><div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}><h3 style={{fontSize:"1rem"}}>Oggi</h3><button className="bt bs bp" onClick={()=>setShowModal("addFood")}>+ Aggiungi</button></div>{todayEntries.length===0?<div style={{textAlign:"center",padding:24,color:"var(--t3)"}}>Nessun pasto</div>:todayEntries.slice(0,5).map(e=>(<div key={e.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",background:"var(--bg2)",borderRadius:8,marginBottom:6,fontSize:".85rem"}}><div><div style={{fontWeight:600}}>{e.food_name}</div><div style={{fontSize:".75rem",color:"var(--t3)"}}>{e.entry_time} - {e.portion}g</div></div><div style={{fontWeight:600,color:"var(--acc)"}}>{e.calories} kcal</div></div>))}</div>
</div></div>);};

const Diet=()=>{const days=["Lunedi","Martedi","Mercoledi","Giovedi","Venerdi","Sabato","Domenica"];const[sel,setSel]=useState(days[new Date().getDay()===0?6:new Date().getDay()-1]);const dp=mealPlan[sel];
...
<div className="cd"><h3 style={{fontSize:"1rem",marginBottom:16}}>Registro</h3>{todayEntries.length===0?<div style={{textAlign:"center",padding:32,color:"var(--t3)"}}>Nessun pasto</div>:todayEntries.map(e=>(<div key={e.id} style={{display:"flex",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid var(--bdr)"}}><div><div style={{fontWeight:600}}>{e.food_name}</div><div style={{fontSize:".75rem",color:"var(--t3)"}}>{e.entry_time} - {e.portion}g</div></div><div style={{textAlign:"right"}}><div style={{fontWeight:600,color:"var(--acc)"}}>{e.calories} kcal</div><div style={{fontSize:".7rem",color:"var(--t3)"}}>P{e.protein} C{e.carbs} G{e.fat}</div></div></div>))}</div></div>);};
...
<h3 style={{fontSize:"1rem",marginBottom:16}}>Badge</h3><div className="g2" style={{marginBottom:32}}>{BADGES.map(b=>(<div key={b.id} className={"bg "+(userStats.days_tracked>=b.req?"ea":"lk")}><div style={{fontSize:"2rem"}}>{b.icon}</div><div><div style={{fontWeight:600}}>{b.name}</div><div style={{fontSize:".75rem",color:"var(--t3)"}}>{b.desc}</div></div></div>))}</div>
...
<div className="cd"><h3 style={{fontSize:"1rem",marginBottom:16}}>Dati</h3>{[{l:"Nome",v:user.name},{l:"Email",v:user.email}].map((x,i)=>(<div key={i} style={{marginBottom:12}}><span className="lb">{x.l}</span><div style={{fontWeight:600}}>{x.v}</div></div>))}</div>
...
<button className="hb" onClick={()=>setSbOpen(!sbOpen)}>{sbOpen?"X":"="}</button>
